...
**`src/tui/app.rs`**
8. Add the binary to `AGENT_COMMANDS` (pane process detection) — only if the pane really reports that name. Check with `tmux display -p '#{pane_current_command}'`, not `ps`: they can disagree, and Node agents usually report `node` (rely on `AGENT_ACTIVE_INDICATORS` there instead)
