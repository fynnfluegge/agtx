...
**Antigravity's trust dialog is deliberately NOT handled by agtx.** `agy` asks "Do you trust the contents of this project?" for every directory it has not seen, and trust is not inherited by subdirectories — so each new worktree prompts. Approving it is the user's decision, not agtx's, so agtx neither pre-seeds `trustedWorkspaces` nor answers the dialog. The consequence is known and accepted: on a task's first launch the skill+prompt send lands while the dialog is up, the text is swallowed by the selector and the Enter accepts the folder, so the agent starts trusted but idle (`⏸`) with no task. Re-triggering the phase (e.g. `r` back to Planning and forward again) re-sends into the now-trusted session.
